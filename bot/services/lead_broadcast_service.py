...
BROADCAST_TIMES = (time(hour=12), time(hour=22, minute=25))
...
class LeadBroadcastService:
    ...
    def schedule_jobs(self, application: Application) -> None:
        ...
        logger.info('Scheduled lead broadcasts at %s and %s (%s)', BROADCAST_TIMES[0], BROADCAST_TIMES[1], self.settings.timezone)
    ...
    async def send_broadcast(self, application: Application, broadcast_hour: int) -> None:
        ...
        if not message_text:
            logger.warning('Lead broadcast was skipped: no broadcast messages configured')
            return
